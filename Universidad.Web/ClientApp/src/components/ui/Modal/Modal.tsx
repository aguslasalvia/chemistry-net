...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children, title }) => {
    ...
    return (
        ...
        >
            <div
                className="flex max-h-[calc(100vh-48px)] w-full max-w-[500px] flex-col overflow-hidden rounded-fq-lg border border-fq-border bg-white shadow-fq-hover animate-slide-up"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="flex shrink-0 items-center justify-between border-b border-fq-border px-6 py-5">
                    {title && <h2 className="font-display text-xl font-bold text-fq-text">{title}</h2>}
                    <button
